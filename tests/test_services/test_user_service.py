...
async def test_get_by_username_user_does_not_exist(db_session):
    retrieved_user = await UserService.get_by_username(
        db_session, "non_existent_username"
    )
    assert retrieved_user is None
...
async def test_create_user_with_invalid_data_returns_error_with_caps_invalid_username(
    db_session,
):
    user_data = {
